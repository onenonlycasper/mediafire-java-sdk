...
public final class MFHttpClientCleanup extends MFHttp {
    ...
    public void returnToken(MFRequest mfRequest, MFResponse mfResponse) {
        MFConfiguration.getStaticMFLogger().logMessage(TAG, "returning token");

        switch (mfRequest.getMfApi().getTokenType()) {
            case SESSION_TOKEN_V2:
                MFConfiguration.getStaticMFLogger().logMessage(TAG, "not returning a token (api was " + mfRequest.getMfApi().getTokenType().toString() + ")");
            ...
            case UNIQUE:
                ...
                if (mfRequest.getMfApi() == MFApi.USER_GET_SESSION_TOKEN) {
                    MFConfiguration.getStaticMFLogger().logMessage(TAG, "returning session token (api was " + mfRequest.getMfApi().getTokenType().toString() + ", " + mfRequest.getMfApi().toString() + " )");
                    ...
                    MFConfiguration.getStaticMFLogger().logMessage(TAG, "returning image action token (api was " + mfRequest.getMfApi().getTokenType().toString() + ", " + mfRequest.getMfApi().toString() + " )");
                    ...
                    MFConfiguration.getStaticMFLogger().logMessage(TAG, "returning upload action token (api was " + mfRequest.getMfApi().getTokenType().toString() + ", " + mfRequest.getMfApi().toString() + " )");
                    ...
                    MFConfiguration.getStaticMFLogger().logMessage(TAG, "not returning a token (api was " + mfRequest.getMfApi().getTokenType().toString() + ", token was class '" + mfRequest.getToken().getClass().getName() + ")");
                }
                break;
            default:
    ...
    public MFImageActionToken createImageActionToken(GetActionTokenResponse imageActionTokenResponse, MFRequest mfRequest) {
        MFConfiguration.getStaticMFLogger().logMessage(TAG, "createImageActionToken()");
        if (imageActionTokenResponse == null) {
            MFConfiguration.getStaticMFLogger().logMessage(TAG, "response was null, returning null");
            return null;
        }

        if (imageActionTokenResponse.hasError()) {
            MFConfiguration.getStaticMFLogger().logMessage(TAG, "response had error, returning null");
            return null;
        }
        ...
        MFConfiguration.getStaticMFLogger().logMessage(TAG, "returning new upload action token");
    ...
    public MFUploadActionToken createUploadActionToken(GetActionTokenResponse uploadActionTokenResponse, MFRequest mfRequest) {
        MFConfiguration.getStaticMFLogger().logMessage(TAG, "createUploadActionToken()");
        ...
        MFConfiguration.getStaticMFLogger().logMessage(TAG, "returning new upload action token: " + mfUploadActionToken.toString());
        return mfUploadActionToken;
    }

    public MFSessionToken updateSessionToken(ApiResponse apiResponse, MFRequest mfRequest) {
        MFConfiguration.getStaticMFLogger().logMessage(TAG, "updateSessionToken()");
        if (apiResponse == null) {
            MFConfiguration.getStaticMFLogger().logMessage(TAG, "response was null, returning original token");
        ...
        if (apiResponse.hasError() && (apiResponse.getErrorCode() == ApiResponse.ResponseCode.ERROR_INVALID_SIGNATURE || apiResponse.getErrorCode() == ApiResponse.ResponseCode.ERROR_INVALID_TOKEN)) {
            MFConfiguration.getStaticMFLogger().logMessage(TAG, "response had error code: " + apiResponse.getErrorCode().toString());
            return null;
        }
        ...
        if (apiResponse.needNewKey()) {
            MFConfiguration.getStaticMFLogger().logMessage(TAG, "needs to recalculate key");
            newToken = ((MFSessionToken) mfRequest.getToken()).getUpdatedSessionToken();
            MFConfiguration.getStaticMFLogger().logMessage(TAG, "original token: " + originalToken.toString());
            MFConfiguration.getStaticMFLogger().logMessage(TAG, "returning new token: " + newToken.toString());
            return newToken;
        }
        MFConfiguration.getStaticMFLogger().logMessage(TAG, "returning original token: " + originalToken.toString());
        return originalToken;
    }

    public MFSessionToken createNewSessionToken(GetSessionTokenResponse newSessionTokenResponse) {
        MFConfiguration.getStaticMFLogger().logMessage(TAG, "createNewSessionToken()");
        ...
        MFConfiguration.getStaticMFLogger().logMessage(TAG, "returning new token: " + mfSessionToken.toString());
