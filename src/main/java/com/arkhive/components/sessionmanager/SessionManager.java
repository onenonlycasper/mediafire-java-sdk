...
public class SessionManager implements SessionRequestHandler {
     ...
     */
    protected void setImageActionToken(ActionTokenResponse value) {
        this.imageActionToken = value;
    }

    /**
     * Set the action token for uploads.
     */
    protected void setUploadActionToken(ActionTokenResponse value) {
        this.uploadActionToken = value;
    }
    ...
    public void releaseSession(Session session) {
        if (sessionList.size() >= maxSessions - 1) {
            return;
        }
        synchronized (this) {
            logger.debug("Returning session token " + session.getSessionToken());
